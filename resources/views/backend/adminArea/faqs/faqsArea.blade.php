            ...
            <table class="table table-centered datatable dt-responsive nowrap table-card-list"
                ...
                @foreach($faqs as $faq)
                    <tr>
                        ...
                        <td>{!! $faq->answer !!}</td>
                        <td>
                            <button type="button" onclick="location.href='{{ url("backend.admin.faq.edit", ["id" => $faq->id]) }}'" class="px-3 text-primary bg-soft-info rounded-1 border-0"><i class="uil uil-edit font-size-18"></i></button>
                            <button type="button" id="deleteFaq" faq-id="{{ $faq->id }}" class="px-3 text-white bg-danger rounded-1 border-0"><i class="uil uil-trash-alt font-size-18"></i></button>
                        </td>
                    </tr>
                @endforeach
                </tbody>
